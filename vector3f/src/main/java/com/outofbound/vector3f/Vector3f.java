package com.outofbound.vector3f;

import android.opengl.Matrix;
import android.util.Log;

import java.io.Serializable;


public class Vector3f implements Serializable {

    public float x,y,z;
    private float[] vector4f;
    private float[] m;
    private Vector3f res;

    public Vector3f(float x, float y, float z){
        this.x = x;
        this.y = y;
        this.z = z;
        vector4f = new float[4];
        m = new float[16];
        res = new Vector3f();
    }

    public Vector3f(float[] v){
        this(v[0],v[1],v[2]);
    }

    private Vector3f(){}

    public Vector3f(Vector3f v){
        this(v.x,v.y,v.z);
    }

    public void multS(Vector3f res, float s){
        res.x = x*s;
        res.y = y*s;
        res.z = z*s;
    }

    public void multS(float s){
        x *= s;
        y *= s;
        z *= s;
    }

    public void cross(Vector3f res, Vector3f v){
        float crossX = y * v.z - z * v.y;
        float crossY = z * v.x - x * v.z;
        float crossZ = x * v.y - y * v.x;
        res.x = crossX;
        res.y = crossY;
        res.z = crossZ;
    }

    public void cross(Vector3f v){
        float crossX = y * v.z - z * v.y;
        float crossY = z * v.x - x * v.z;
        float crossZ = x * v.y - y * v.x;
        x = crossX;
        y = crossY;
        z = crossZ;
    }

    public float dot(Vector3f v){
        return x * v.x + y * v.y + z * v.z;
    }

    public void normalize(){
        float length = length();
        x /= length;
        y /= length;
        z /= length;
    }

    public void normalize(Vector3f res){
        float length = length();
        res.x = x/length;
        res.y = y/length;
        res.z = z/length;
    }

    private void to4f(){
        vector4f[0] = x;
        vector4f[1] = y;
        vector4f[2] = z;
        vector4f[3] = 1;
    }

    private void to3f(){
        x = vector4f[0];
        y = vector4f[1];
        z = vector4f[2];
    }

    public void multiplyMV(float[] m, Vector3f res){
        to4f();
        Matrix.multiplyMV(res.vector4f,0, m,0,vector4f,0);
        res.to3f();
    }

    public float[] multiplyMV4f(float[] m, Vector3f res){
        to4f();
        Matrix.multiplyMV(res.vector4f,0, m,0,vector4f,0);
        res.to3f();
        return res.vector4f;
    }

    public void rotate(float a, float x, float y, float z){
        Matrix.setIdentityM(m,0);
        Matrix.rotateM(m,0,a,x,y,z);
        multiplyMV(m,this);
    }

    public void rotate(Vector3f res, float a, float x, float y, float z){
        Matrix.setIdentityM(m,0);
        Matrix.rotateM(m,0,a,x,y,z);
        res.copy(this);
        multiplyMV(m,res);
    }

    public void translate(Vector3f shift){
        Matrix.setIdentityM(m,0);
        Matrix.translateM(m,0,shift.x,shift.y,shift.z);
        multiplyMV(m,this);
    }

    public void translate(Vector3f res, Vector3f shift){
        Matrix.setIdentityM(m,0);
        Matrix.translateM(m,0,shift.x,shift.y,shift.z);
        res.copy(this);
        multiplyMV(m,res);
    }

    public float length(){
        return (float)Math.sqrt(x*x + y*y + z*z);
    }

    public void add(Vector3f res, Vector3f v){
        res.x = x + v.x;
        res.y = y + v.y;
        res.z = z + v.z;
    }

    public void add(Vector3f v){
        x += v.x;
        y += v.y;
        z += v.z;
    }

    public void sub(Vector3f res, Vector3f v){
        res.x = x - v.x;
        res.y = y - v.y;
        res.z = z - v.z;
    }

    public void sub(Vector3f v){
        x -= v.x;
        y -= v.y;
        z -= v.z;
    }

    public float angleXY(){
        if (x == 0){
            if (y > 0) {
                return 90;
            }
            else if (y < 0){
                return 270;
            }
            return 0;
        }
        else {
            float a = (float)Math.atan(y/x);
            a = (float)Math.toDegrees(a);
            if (x < 0) {
                a += 180f;
            }
            return a;
        }
    }

    public float distance(Vector3f v){
        res.x = x-v.x;
        res.y = y-v.y;
        res.z = z-v.z;
        return res.length();
    }

    public void copy(Vector3f v){
        x = v.x;
        y = v.y;
        z = v.z;
    }

    public Vector3f clone(){
        return new Vector3f(x,y,z);
    }

    public void print(String tag){
        Log.i(tag,"x "+x);
        Log.i(tag,"y "+y);
        Log.i(tag,"z "+z);
    }
}
